...
public:
    ...
    int numberOfGreens = 0;
    int  numberOfCardsInColor[COLOR_MAX] = { 0, 0, 0, 0, 0, 0
    }; // tablica przetrzumująca iloś kart w danym kolorze 0 -zielony, 1 - niebieskli itd;
    int valueOfGreens[CARDS_VALUE] = {
    }, valueOfBlues[CARDS_VALUE] = {
    }, valueOfReds[CARDS_VALUE] = {
    }, valueOfViolets[CARDS_VALUE] = {
...
void giveCardsToPlayers(Player* players, int numberofPlayers, FILE* f) {
    for (int i = 0; i < numberofPlayers; i++) { // rozdaje karty graczom, na ręke i na stosik przed nich
...
void giveCardsToPiles(Pile* piles, int numberOfPiles, FILE* f) {
    for (int i = 0; i < numberOfPiles; i++) { // rozdaję karty do kociołków
...
int countGreens(Player* players, Pile* pile, int numberofPlayers, int numberOfPiles) {
    int numberOfGreens = 0;
    int* tmp = new int[numberofPlayers];// tablice tymczsowe,, które posłóżą do dodania wartości
    ...
    int tmp3 = 0;
    for (int i = 0; i < numberofPlayers; i++) {
        tmp1[i] += players[i].deck.numberOfGreens;// dodaje osobno ze stosiku i z ręki gracza, liczba zielnoych kart  w późniejszej fazie gry musi być niezależna
    ...
    for (int i = 0; i < numberOfPiles; i++)
        numberOfGreens += pile[i].numberOfGreens;// do zielonych kart graczy dodaje karty z kociołków
...
int saveAndCheckGreensValue(Player* players, Pile* piles, int numberOfPlayers, int NumberOfPiles, int numberofGreens) {
    ...
    for (int i = 0; i < numberOfPlayers; i++) {
        for (int j = 0; j < players[i].handCardNumber; j++) {
            if (strcmp(players[i].hand.cards[j].color, "green") == 0) {// sprawdzam ktory kolor to green i wartości tych kart wpisuje do tablicy 
    ...
    int i = 0;
    int first = greensValue[i];// porównuje tablice i sprawdzam czy wszystkie jej elementy są równe pierwszemu 
...
int* countNumberOfColors(Player* players, Pile* piles, int numberOfPlayers, int numberOfPiles) {
    ...
    int* color = new int[6];
    for (int i = 0; i < numberOfPlayers; i++) {// zliczam poszczególne kolory z rąk i stosów wszystkich graczy
    ...
    for (int i = 0; i < numberOfPiles; i++) {//zliczam kolory z kociołków
    ...
    color[0] = bluePlayerHand + bluePlayerDeck + bluePile;//dodaje je i wpisuje do tablicy celem sprawdzania później tej tablicy wzdłuż i w szerz
...
int checkColor(Player* players, Pile* piles, int numberOfPlayers, int numberOfPiles) {
    ...
    int i = 0;
    for (; i < COLOR_MAX; ++i) // sprawdzamy czy wszystkie wartości są takie same
...
int countValues(Player* players, int numberOfPlayers, Pile* piles, int numberOfPiles) {
    ...
    for (int i = 0; i < numberOfPlayers; i++) {
        for (int j = 0; j < players[i].handCardNumber; j++) {
            if (strcmp(players[i].hand.cards[j].color, "blue") == 0) {// sprawdzam wartości poszczególnyuch kolorów i zapisuje je do tablicy dwu wymiarowej
    ...
    for (int i = 0; i < numberOfPlayers; i++) {                                              //dodaje do istniejącej tablicy warości i kolorów wartości i kolory ze stosików graczy
        for (int j = 0; j < players[i].deckCardNumber; j++) {
    ...
    for (int j = 0; j < numberOfPiles; j++) {        //dodaje do istniejącej tablicy warości i kolorów wartości i kolory ze kociołków
    ...
    colors = sort(colors, sizes); // sortuje tablice dwuwymiarową
    for (int i = 0; i < COLOR_MAX; i++) {
        for (int j = 0; j < sizes[i]; j++)
            if (colors[0][j] == colors[i][j] || (colors[0][j] == colors[i][j] && colors[i][j] == 0))//porównuje czy eszystkie wartości są takie sdame
                check = 1;
            else {
                check = 0;
...
int checkCardOnHand(Player* players, int numberOfPlayers) {
    int check = 1;
    for (int i = 0; i < numberOfPlayers; i++) {// odejmuje liczbe kart każdego gracza od każdego i jeżeli różnica między nimi jest mneijsza niż -1 lub więsza niż 1
        for (int j = i - 1; j >= 0; j--) {               // to znaczy że któeyś gracz ma za dużo lub za mało kart
...
void endRound(Player* players, int numberOfPlayers, FILE* file) {
    ...
    };//w rzędach odpowiadających kolorm, będzie numer gracza odpornego na dany kolor(immunity[0] = niebieski, immunity[1] - czerwony itp)
    int playersColor[6][COLOR_MAX] = { 0, 0, 0, 0, 0, 0
    };  //tworze tablice 2D która będzie zestawiała kolory każdego gracza i sprawdzała gdzie wartość maksymalna
    int max[6] = { 0, 0, 0, 0, 0, 0
    };
    int check[6] = { 0, 0, 0, 0, 0, 0
    };             //używam tablicy aby uniknąć 12 różnych zmiennych
    ...
    for (int i = 0; i < numberOfPlayers; i++) {
        for (int j = 0; j < COLOR_MAX; j++) {
            if (max[j] < playersColor[i][j]) {// tavbela max numer indexu = color, wartość = maxymalna wartość w kolorze
...
void shuffle_deck(Deck& deck)
{
    ...
    for (i = 0; i < deck.cardsNumber; i++) {
        ...
        tmp.cards[0] = deck.cards[i]; // zamień cards[i] z cards[j]
